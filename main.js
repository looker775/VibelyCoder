...
const fetch = require('node-fetch'); // ✅ Claude API support
...
function createMainWindow() {
  ...
  // ✅ Start with LICENSE screen first
  mainWindow.loadFile('license.html').then(() => {
    console.log("✅ license.html loaded.");
  }).catch(err => console.error("❌ Error loading license.html:", err));
...
ipcMain.handle('verify-license', async (event, key) => {
  ...
  console.log(`🔑 Checking license key: ${key}`);
...
ipcMain.on('license-success', () => {
  if (mainWindow) {
    console.log("✅ License accepted — loading main index.html");
